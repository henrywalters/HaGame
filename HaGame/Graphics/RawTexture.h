...
namespace hagame {
	namespace graphics {
		...
		template <GLuint TextureType, GLuint DataType = GL_FLOAT>
		class RawTexture {
		public:
		...
		protected:
			...
			void initialize(Vec2Int size, T* data) {
				glGenTextures(1, &id);
				glBindTexture(GL_TEXTURE_2D, id);
				glTexImage2D(GL_TEXTURE_2D, 0, TextureType, size[0], size[1], 0, GL_RGBA, DataType, data);
			...
			void initialize(Vec2Int size) {
				glGenTextures(1, &id);
				glBindTexture(GL_TEXTURE_2D, id);
				glTexImage2D(GL_TEXTURE_2D, 0, TextureType, size[0], size[1], 0, GL_RGBA, DataType, NULL);
				setParameter(GL_TEXTURE_MIN_FILTER, GL_NEAREST);
				setParameter(GL_TEXTURE_MAG_FILTER, GL_NEAREST);
			}
		};
