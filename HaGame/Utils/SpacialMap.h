#ifndef SPACIAL_MAP_H
#define SPACIAL_MAP_H
...
namespace hagame {
	namespace utils {
		...
		template <class DataType, class SpacialDataType>
		class SpacialMap {
		...
		public:
			...
			hagame::math::Vector<3, SpacialDataType> chunkSize;

			SpacialMap(hagame::math::Vector<3, SpacialDataType> _chunkSize) : map(Map<long, Array<DataType*>>()), chunkSize(_chunkSize) {}

			void clear() {
				map.clear();
			}

			void insert(hagame::math::Vector<3, SpacialDataType> position, DataType* value) {
			...
			Array<DataType*> get(hagame::math::Vector<3, SpacialDataType> position) {
			...
			long hashPosition(hagame::math::Vector<3, SpacialDataType> position) {
