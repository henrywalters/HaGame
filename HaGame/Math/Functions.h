...
public:
	...
	static LineIntersection Collinear(hagame::math::Interval<float> tInterval, Vec2 a, Vec2 b) {
		LineIntersection intersection;
		intersection.m_collinear = true;
		intersection.m_intersected = true;
		intersection.m_tInterval = tInterval;
		intersection.m_segment = hagame::math::LineSegment(a, b);
		return intersection;
	}
	...
	hagame::math::LineSegment segment() {
		if (!m_collinear) throw new std::exception("Intersection is not collinear");
		return m_segment;
	}

	hagame::math::Interval<float> tInterval() {
		if (!m_collinear) throw new std::exception("Intersection is not collinear");
		return m_tInterval;
	}

	Vec2 pos() {
		if (!m_intersected) throw new std::exception("Non-intersection");
		if (m_collinear) throw new std::exception("Collinear - use segment instead");
		return m_pos;
	}
...
private:
	...
	float m_t;
	hagame::math::Interval<float> m_tInterval;
	Vec2 m_pos;
	hagame::math::LineSegment m_segment;
};
...
bool linesIntersect(Vec2 a, Vec2 b, Vec2 p, Vec2 q, LineIntersection& intersection) {
	...
	auto angleA = atan2(s1.y(), s1.x());
	auto angleB = atan2(s2.y(), s2.x());
	...
	float num1 = (a.x() - p.x()) * (p.y() - q.y()) - (a.y() - p.y() * (p.x() - q.x()));
	float den1 = (a.x() - b.x()) * (p.y() - q.y()) - (a.y() - b.y()) * (p.x() - q.x());

	float num2 = (a.x() - p.x()) * (a.y() - b.y()) - (a.y() - p.y()) * (a.x() - b.x());
	float den2 = (a.x() - b.x()) * (p.y() - q.y()) - (a.y() - b.y()) * (p.x() - q.x());

	if (den1 < 0.0001 || den2 < 0.0001) {
		return false;
	}
	...
	*/
	...
	if (denom == 0 && collinearTest) {
		...
		auto t = hagame::math::Interval<float>(t0, t1);
		if (t.overlaps(hagame::math::Interval<float>(1.0))) {
